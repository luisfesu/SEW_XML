...
file = open("arbolGenealogico.html", "w") # Abre y borra automaticamente el contenido del archivo
...
def parse_fotos(data, familiar):
    ...
    write_in_file("<h4>Fotos del familiar</h4>")

    _foto_index = 1
    for retrato in data.findall("./retrato"):
        alt = "foto " + str(_foto_index) +" de " + str(familiar.attrib.get("nombre"))
        write_in_file('<img src="multimedia/{0}" alt="{1}"/><br />'.format(retrato.text, alt))
...
def parse_videos(data):
    ...
    if len(videos) != 0:
        write_in_file("<h4>Videos del familiar</h4>")

    for video in videos:
        write_in_file('<video controls>')
        write_in_file('\t<source src="multimedia/{0}" type="video/mp4"/><br />'.format(video.text))
        write_in_file('</video>')


def parse_comentarios(data):

    comentarios = data.findall("./comentario")

    if len(comentarios) > 0:
        write_in_file("<h4>Comentarios del familiar</h4>")

    """ Convertir elemento xs:comentario a HTML <p>"""
    for coment in comentarios:
        write_in_file('<p>')
        write_in_file(coment.text)
        write_in_file('</p>')
